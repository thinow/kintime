...
- [x] 1. Schema — `moments` migration: `id UUID PK`, `user_id FK → users`, `kin_id FK → kin`, `duration_minutes INT NOT NULL`, `logged_at timestamptz NOT NULL DEFAULT now()`, `created_at timestamptz NOT NULL DEFAULT now()`. No backend or frontend changes. Verify: Neon console shows `moments` with correct columns and FK constraints.
