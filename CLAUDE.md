...
- [ ] 4a. Email validation — **backend**: add `email-validator` dep, enforce valid email format in `POST /auth/request-token` (422 on bad input). Backend tests. Verify: `curl` with `"not-an-email"` → `422`.
- [ ] 4b. Resend wiring — **backend**: add `resend` dep, add `RESEND_API_KEY` to Fly.io secrets (manual), send magic link email in `request_token`. Tests mock the Resend call. Verify: curl → `204` and email lands in inbox.
