...
- [ ] 4. Email delivery — **backend**: add `resend` + `email-validator` deps, validate email format in `POST /auth/request-token`, wire magic link email into token endpoint. `RESEND_API_KEY` added to Fly.io secrets. Deployable: curl → real email lands in inbox.
